const fs = require('fs'),
    ...
    filePathToFetchCalendar = path.join("test/Rodrigo Franco's Notes — Primary Interests_Calendar.html"), // Iteration here for multiple files
    filePathToSaveCalendar = path.join("test/Rodrigo Franco's Notes — Primary Interests.html")
...
function writeTemplateToHTML(_template) {
    ...
    fs.readFile(filePathToSaveCalendar, { encoding: 'utf-8' }, function (err, data) {
        ...
        if (!err) {
            ...
            console.log("Calendars Templates Written Successfully!");
        }
    })
}
